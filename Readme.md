...
3. Настройте подключение к базе данных. **Строка подключения (`connection_string`) является обязательным параметром** и не имеет значения по умолчанию. Вы можете задать ее одним из следующих способов:
    *   **Файл конфигурации (рекомендуется для безопасности):**
        Добавьте в ваш файл `mssql2file.cfg` (или другой указанный через флаг `-config`):
        ```json
        {
          "Connection_string": "server=your_server;port=1433;user id=your_user;password=your_password;database=your_database;TrustServerCertificate=true;encrypt=disable;"
          // ... другие параметры
        }
        ```
    *   **Флаг командной строки:**
        Используйте флаг `-connection_string`:
        ```bash
        mssql2file -connection_string "server=your_server;..." [другие_параметры]
        ```
    *   **Переменная окружения:**
        Задайте переменную `M2F_CONNECTION_STRING`:
        ```bash
        export M2F_CONNECTION_STRING="server=your_server;..."
        mssql2file [другие_параметры]
        ```
4. (Необязательно) Создайте или дополните файл конфигурации `mssql2file.cfg` другими параметрами.

### Использование:
1. Откройте командную строку и перейдите в директорию, где находится исполняемый файл MSSQL2File.
2. Убедитесь, что строка подключения к базе данных настроена (см. раздел "Установка").
3. Введите команду `mssql2file` с нужными параметрами.
4. MSSQL2File экспортирует данные из базы данных в файл.

### Логирование:
Утилита использует структурированное логирование в формате JSON, которое выводится в `stderr`. Основные события логируются на уровне INFO. Флаг `-silient` был удален.
